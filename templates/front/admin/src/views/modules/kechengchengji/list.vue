<template>
	<div class="main-content" :style='{"color":"#3B3E40","padding":"30px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"0 10px","margin":"0 0 20px","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e3e3e3","padding":"10px 0","borderRadius":"5px","textAlign":"center","flexWrap":"wrap","background":"#fff","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-start","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">年级</label>
						<el-input v-model="searchForm.nianji" placeholder="年级" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">班级</label>
						<el-input v-model="searchForm.banji" placeholder="班级" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' class="select" label="课程科目" prop="kechengkemu">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">课程科目</label>
						<el-select clearable v-model="searchForm.kechengkemu" placeholder="请选择课程科目" >
							<el-option v-for="(item,index) in kechengkemuOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"border":"0px solid #e3e3e3","padding":"10px 0","margin":"0","color":"#fff","textAlign":"left","flexDirection":"row","display":"flex","justifyContent":"flex-start","borderRadius":"5px","flexWrap":"wrap","background":"#fff","width":"auto","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('kechengchengji','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('kechengchengji','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('kechengchengji','学生成绩统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						学生成绩统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('kechengchengji','课程合格统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						课程合格统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #e9e9e9","width":"100%","padding":"0px","margin":"10px 0 0","borderRadius":"5px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- $column.hiden -->
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="chengjidan" width="200" label="成绩单">
						<template slot-scope="scope">
							<div v-if="scope.row.chengjidan">
								<img v-if="scope.row.chengjidan.substring(0,4)=='http'" :src="scope.row.chengjidan.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.chengjidan.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kechengchengji','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="edit" v-if=" isAuth('kechengchengji','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kechengchengji','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="kechengfenshuChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var kechengfenshuChart1 = echarts.init(document.getElementById("kechengfenshuChart1"),'vintage');
        this.$http({
            url: `kechengchengji/value/xueshengxingming/kechengfenshu`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                kechengfenshuChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kechengfenshuChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var shifouhegeChart2 = echarts.init(document.getElementById("shifouhegeChart2"),'vintage');
        this.$http({
            url: "kechengchengji/typeStat/shifouhege/xueshengxingming",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].xueshengxingming);
                    yAxis1.push(parseFloat((res[i].合格)));
                    yAxis2.push(parseFloat((res[i].不合格)));
                    pArray.push({
    ...
    init () {
          this.$http({
            ...
            if (data && data.code === 0) {
              this.kechengkemuOptions = data.data;
            } else {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.kechengkemu!='' && this.searchForm.kechengkemu!=undefined){
            params['kechengkemu'] = this.searchForm.kechengkemu
          }
			this.$http({
	...
	kechengchengjistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kechengchengji/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
